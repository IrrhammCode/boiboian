...
export default function (THREE) {
  ...
  HAIR.name = 'fabric';
  const SHIRT = new THREE.MeshStandardMaterial({ color: 0xf0a020, roughness: 0.9, metalness: 0 });
  SHIRT.name = 'fabric';
  ...
  head.add(hair);
  const spike = new THREE.Mesh(new THREE.ConeGeometry(0.05 * S, 0.11 * S, 6), HAIR);
  spike.position.set(0.02 * S, 0.18 * S, -0.02 * S);
  head.add(spike);
