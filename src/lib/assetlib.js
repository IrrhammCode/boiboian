 ...
 */
/**
 * Vite-friendly path: pass the default export factory from an asset module.
 * Prefer this over URL strings so the bundler can resolve assets.
 */
export async function ASSET_FACTORY(fn, opts = {}) {
  const key = `fn:${opts.cacheKey || fn.name || 'asset'}:${!!opts.keepHierarchy}`;
  if (!cache.has(key)) {
    cache.set(key, (async () => {
      const built = fn(THREE);
      const merged = opts.keepHierarchy ? built : mergeByMaterialValues(built);
      const box = new THREE.Box3().setFromObject(merged);
      const c = box.getCenter(new THREE.Vector3());
      merged.position.set(-c.x, -box.min.y, -c.z);
      const wrapper = new THREE.Group();
      wrapper.add(merged);
      wrapper.userData.nativeSize = box.getSize(new THREE.Vector3());
      if (opts.keepHierarchy) carryDeclarations(built, wrapper);
      return wrapper;
    })());
  }
  try {
    ...
    console.warn('[assets] factory', e.message);
